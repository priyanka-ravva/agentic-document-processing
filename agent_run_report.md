...
- Evaluation checks required-field presence but does not yet compare extracted values against gold expected values.
- The planner uses an LLM-assisted decision first, but keeps deterministic metadata rules as a reliability fallback. This improves assignment alignment while preserving predictable behavior when the model or provider is unavailable.
- Vision fallback for PDFs currently renders only the first page, so multi-page scanned PDFs still rely primarily on OCR.
- Chunked extraction currently targets searchable PDF text. OCR still processes scanned PDFs into one text stream before extraction.
- OCR quality depends on the local Tesseract installation and image quality.
- The current system is optimized for three document families rather than arbitrary document types.
